...
# scatter + rug plot ----
p1 +
  geom_rug() #helps us visualize the distribution of the data 
```
...
ggExtra::ggMarginal(p2, type = "density", groupFill = TRUE, groupColour = TRUE) #groupFill = different color by site, groupColour = outline
```
...
stream_chem_data |> 
  ...
  ggplot(aes(x = DOC, y = pH)) + 
  geom_point(alpha = 0.5) +
  geom_smooth() #less than 1000 --> uses default which draws a line for moving average; helpful for exploring data but you shouldn't use as final line. When you specify 'lm' it will fit a linear model even if it is not linear. 
...
stream_chem_data |> 
  ...
  ggplot(aes(x = DOC, y = pH, color = site, size = Al_ICP)) + 
  geom_point(alpha = 0.5) +
  scale_size(range = c(1, 10)) + # always use scale_size()... don't use scale radius --> scales by diameter and can be deceiving 
  labs(x = "DOC (mg/L)", size = "Al (mg/L)", color = "Site")
```

...
#..........................load packages.........................
library(tidyverse) 
library(scales) # makes labels easier to read
...
jobs_clean <- jobs |> 
  ...
  # new column called group label 
  # anything that's not in those groupings gets a NA
...
jobs_clean |> 
  ...
  mutate(occupation = fct_reorder(occupation, total_earnings)) %>% # reorders occupations by total earnings (more for visual)
  
      # set up plot --> dont need to define data as we are in the same code chunk 
  ggplot(aes(x = occupation, y=total_earnings)) +
  geom_col() +
  geom_text(aes(label = dollar(total_earnings)), #label_currency() does not work with geom_text()
            hjust = 1.2, color = "white") + #hjust = horizontal justification
  scale_y_continuous(labels = label_currency(scale = 0.001, suffix = "k")) + #determine what type of data is on the y axis --> this case its continuous
  coord_flip()
...
jobs_clean |> 
  ...
  geom_point() +
  geom_text(aes(label = dollar(total_earnings)), #label_currency() does not work with geom_text()
            hjust = -0.2) + #hjust = horizontal justification
  geom_linerange(aes(ymin = 0, ymax = total_earnings)) + #needs aes, start our line at 0 and extend to total earnings
 scale_y_continuous(labels = scales::label_currency(accuracy = 1, scale = 0.001, suffix = "k"),
                     limits = c(0, 235000)) + # expand axis to make room for values
  coord_flip() +
  theme_minimal()
```
...
- use `geom_col()` when you have data that's already summarized (aka we dont need ggplot to do any calculations) - make the bar proportional to the total earning columns. 
...
jobs_clean |> 
  ...
  geom_col(position = position_dodge()) + #stack the bars side by side 
  coord_flip()
```

### Lollipop chart (2 groups)
truncated axes leads viewers to perceive illustrated differences as larger or more importnat than they actually are ---
...
jobs_clean |>
  ...
  geom_linerange(aes(xmin = occupation, xmax = occupation, 
                 ...
                 position = position_dodge(width = 0.5)) + #width lets you add the second bar
  coord_flip() +
  theme_minimal()
```
...
- total earnings: numeric, continuous
...
analysts <- jobs_clean |> 
  filter(str_detect(string = occupation, pattern = "analyst")) |>  #look in occupation column for anything w the word analyst in it
...
ggplot(heatmap_order, aes(x = year, y = occupation, fill = total_earnings)) +
  ...
  labs(fill = "Total Earnings", 
       x = "",
       y = "" ) + 
  coord_fixed() + # each unit on y axis = unit on x axis
  scale_fill_viridis_c(labels = label_currency(scale = 0.001, suffix = "k")) + # c = continuous data 
  scale_y_discrete(label = label_wrap(30)) +
  guides(fill = guide_colorbar(barwidth = 15, barheight = 0.75,
                               title.position = "top")) + #guides() used to adjust legends, colorbar refers to gradient
  theme_minimal()+
  theme(
    legend.position = "top"
    ) 
...
ggplot(subset_jobs) +
  ...
  geom_linerange(aes(y = occupation, 
                       ...
                       xmax = total_earnings_male)) + #put the lines before the points

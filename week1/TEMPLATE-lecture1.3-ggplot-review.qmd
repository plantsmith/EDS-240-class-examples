...
```{r}
#| warning: false
# aes - think, aesthetically i want to see...blah on x, etc. 
...
#you get 3 lines of best fit, becaue you defined color as species in the global layer, which applies to all subsequent layers
```
...
ggplot(penguins, aes(x = species)) +
  geom_bar()+
  theme_minimal()
#count is default statistical transformation --> makes the height of the bar proportional to the number of cases in each group (or if the weight aesthetic is supplied
```
...
ggplot(penguin_summary, 
       aes(x = species))+
  geom_bar()
#default stat is count and there is only one observation in the df
...
ggplot(penguins, aes(x = flipper_length_mm, fill = species)) +
  ...
  scale_fill_manual(values = c("darkorange","purple", "dodgerblue")) +
  labs(
    x = "Flipper Length (mm)", 
    y = "Frequency",
    fill = "Species", 

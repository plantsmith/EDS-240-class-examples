...
ggplot(lyme_pop, aes(x = year, y = cases_per100k, group = state, color = state)) +
  geom_line() +
  gghighlight::gghighlight(max(cases_per100k) > 100) #highlight every state that goes over 100
```
...
lyme_pop |> 
  ...
  ggplot(aes(x = year, y = cases_per100k, group = state)) +
  geom_line() +
  scale_y_continuous(limits = c(0, NA)) # force y-axis orgin to 0
```

### Note 2: Consider aspect ratio

-   Aspect ratio = height to width ratio of plot
...
lyme_pop |> 
  ...
  geom_line() +
  scale_y_continuous(breaks = seq(0, 190, by = 1)) 
  # coord_fixed(ratio = 1) # each unit on y is the same as x --> we dont want this in this instance
```
...
lyme_pop |> 
  ...
  ggplot(aes(x = year, y = cases_per100k, group = state)) +
  geom_line() +
  coord_fixed(ratio = 1/50) #1 unit of height = 50 units of width
```
...
#represented as difference in size
```
...
lyme_pop |> 
  filter(state %in% c("Maine", "Rhode Island", "New Hampshire", "Vermont")) |> 
  ggplot(aes(x = year, y = cases_per100k, group = state, fill = state)) +
  geom_area(position = "fill") #default is stack

# we can see the relative contributions of state count 
```
...
lyme_pop |> 
  ...
  facet_wrap(~state)
```
want to show groups though time --> line chart
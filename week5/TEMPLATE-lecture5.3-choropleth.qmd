...
precip_wrangled <- precip_data |>
  ...
  # keep only US states (this will filter out DC) ---- # REAL STATES ONLY
...
# the zero is not falling at the neutral color, but the blue --> insinuating things are wetter
                       values = scales::rescale(x = c(-40, 0, 20)) 
) + 
  guides(fill = guide_colorbar(barwidth = 25, barheight = 0.75)) #stretch out legend
...
base_map + 
  ...
  guides(fill = guide_colorsteps(barwidth = 25, barheight = 0.75)) #stretch out legend

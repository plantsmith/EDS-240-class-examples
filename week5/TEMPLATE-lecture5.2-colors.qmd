...
# for classed continuous color scales ---- aka broken into bins
con_color_plot +
...
- google ggplot shapes --> choose 21 which is a fillable circle
...
# update color --> fill
# update scale_color_ --> scale_fill_..
...
# add shape by species to help differenciate the groups
...
ggplot(na.omit(penguins), aes(x = species, fill = sex)) +
  geom_bar() +
  scale_fill_hue(c = 70) # decrease the saturation 
```
...
ggplot(na.omit(penguins), aes(x = bill_length_mm, y = bill_depth_mm, color = sex)) +
  geom_point() +
  scale_color_hue(l = 45) # luminance/lightness
...
ggplot(penguins, aes(x = species)) +
  geom_bar(fill = "#00FF33", color = "gray7", alpha = 0.5) + # alpha tones down the opacity 
  theme_classic() 
```
...
penguins_ex1 <- penguins |> 
  mutate(
    my_color = case_when( # adding a column for color
...
ggplot(penguins_ex1, aes(x = bill_length_mm, y = bill_depth_mm, color = my_color)) + # color = my color 
  geom_point(size = 4, alpha = 0.8) +
  scale_color_identity() # extracts colors from data frame and maps to appropriate data points
```
...
ggplot(penguins_ex2, aes(x = bill_length_mm, y = bill_depth_mm, color = my_color)) +
  geom_point(size = 4, alpha = 0.8) +
  # applies colors from my color
  scale_color_identity(guide = "legend", # create a legend
                       name = "Body mass (g)", #update legend name
                       labels = c(">6000", "<= 6000")) #define what labels are
```
